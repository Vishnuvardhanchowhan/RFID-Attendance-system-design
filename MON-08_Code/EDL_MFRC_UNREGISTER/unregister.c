        ...
        if(status == MI_OK){
            ...
            if(status == MI_OK){
                ...
                if(!deregister){
                    GPIOPinWrite(GPIO_PORTF_BASE, redLED, redLED);
                    lcd_init();
                    lcd_string("De-registration");
                    lcd_cursor(2,1);
                    lcd_string("Unsuccessful");
                    SysCtlDelay(SysCtlClockGet()); //Delay 3 sec
                    GPIOPinWrite(GPIO_PORTF_BASE, redLED, 0);
                    te = ReadReg(TxControlReg);
                    if(te==131){
                ...
                if(status==MI_OK){
                    ...
                    if(status!=MI_OK){
                        ...
                        lcd_cursor(2,1);
                        lcd_string("De-registered");
                        if(i!=numcards){
                            EEPROMRead(&IDstored, numcards*4, 4);
                            EEPROMProgram(&IDstored, i*4, 4);
                        }
                        numcards--;
